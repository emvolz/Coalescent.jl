#= 
Test the simple function-based DSL implementation (corrected)
=#
...
try
    sir_model = create_model("SIR_Functional", function(model)
...
println("\\nTest 2: SEIR Model...")
try
    seir_model = create_model("SEIR_Functional", function(model)
        parameter!(model, :β, 3.0)
...
try
    meta_model = create_model("Metapopulation_Functional", function(model)
...
try
    create_model("Bad_Model", function(model)
        parameter!(model, :β, 3.0)
...
try
    create_model("Bad_Model2", function(model)
        parameter!(model, :β, 3.0)
        deme!(model, :I, 1.0)
